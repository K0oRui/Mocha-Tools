...
def get_background() -> str:
    ...
    try:
        ...
        write_debug_log(f"[Silenced] get_background: {e}")
...
def set_background(name: str, persist: bool = True) -> None:
    ...
    if persist:
        try:
            ...
            write_debug_log(f"[Silenced] set_background: {e}")
    ...
    try:
        ...
        if v:
            ...
            try:
                ...
                write_debug_log(f"[Silenced] get_accent: {e}")
                return str(v)
    except (AttributeError, TypeError, RuntimeError) as e:
        write_debug_log(f"[Silenced] get_accent: {e}")
...
class _AccentNotifier(QObject):
    # Carries the old and new accent hex values.
    accent_changed = Signal(str, str)
    # Carries the font family and size.
    font_changed = Signal(str, int)
    # Carries the old and new theme keys.
...
def set_accent(accent_hex: str, persist: bool = True) -> None:
    ...
    try:
        ...
        write_debug_log(f"[Silenced] set_accent: {e}")
    # normalize
...
def get_font() -> tuple[str, int]:
    ...
    global _current_accent
    try:
        if "_current_font_family" in globals() and globals().get(
        ...
        write_debug_log(f"[Silenced] get_font: {e}")
    try:
        s = QSettings(ORG_NAME, APP_NAME)
...
def set_font(family: str, size: int, persist: bool = True) -> None:
    ...
    _old_family, _old_size = get_font()
    # normalize
    fam = family or DEFAULT_FONT_FAMILY
    try:
        sz = int(size)
    except (AttributeError, TypeError, RuntimeError, ValueError) as e:
        write_debug_log(f"[Silenced] set_font: {e}")
