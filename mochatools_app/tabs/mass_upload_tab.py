"""
tabs/mass_upload_tab.py — Queue-based multi-file uploader tab.
...
from ..constants import DEFAULT_CHUNK_SIZE_MB, DEFAULT_MAX_CHUNKS, HARDCODED_BASE_URL
...
class MassUploadTab(QWidget):
    ...
    def __init__(self, get_api_key, get_mass_settings=None, get_debug=None,
                 on_upload_done=None, parent=None):
    ...
    def _build_drop_section(self, parent_lay: QVBoxLayout):
        ...
        parent_lay.addWidget(self._sh("Queue"))
    ...
    def _queue_context_menu(self, pos):
        ...
        if item:
            ...
            menu.addAction("Clear done", self._clear_done)
    ...
    def _move_entry(self, delta: int):
        ...
        for entry in selected:
            ...
            if new_ti < 0 or new_ti >= self._tree.topLevelItemCount():
                continue
            if (entry["status"] == "uploading" or self._queue[new_qi]["status"] == "uploading"):
                continue
    ...
    def _start(self):
        ...
        for slot in range(total_slots):
            if slot == 0:
                ...
                # Stagger concurrent workers by 1.5 s to avoid hammering the server
    ...
    def _on_file_done(self, entry: dict):
        ...
        # Notify cache that this upload destination needs a refresh
        if self._on_upload_done_cb is not None:
            dest = entry.get("dest", "")
            if dest:
                # Pass the folder portion (everything before the filename)
